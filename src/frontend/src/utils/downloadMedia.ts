/**
 * Media download utilities with format preservation.
 * - Uploaded images are downloaded in their original format
 * - Embedded URL images are converted to PNG
 * - Videos are downloaded as MP4
 */

import type { ExternalBlob, MediaFile } from "../backend";
import { getExtensionFromMimeType } from "./mime";

/**
 * Downloads an uploaded image in its original format using stored metadata.
 * Preserves the original filename and content type.
 */
export async function downloadUploadedImage(
  mediaFile: MediaFile,
  fallbackFilename = "image.jpg",
): Promise<void> {
  try {
    // Get image bytes
    const imageBytes = await mediaFile.file.getBytes();

    // Create blob with original content type
    const blob = new Blob([imageBytes], { type: mediaFile.contentType });

    // Use original filename or fallback
    const filename = mediaFile.originalName || fallbackFilename;
    ...
    console.error("Error downloading uploaded image:", error);
    throw error;
  }
}
 ...
 * Used for embedded/linked images from URLs.
 */
export async function downloadImageAsPNG(
  ...
  try {
    ...
    const anchor = document.createElement("a");
    anchor.href = url;
    anchor.download = filename.endsWith(".png")
      ? filename
...
export async function downloadVideoAsMP4(
  ...
  try {
    ...
    const anchor = document.createElement("a");
    anchor.href = url;
    anchor.download = filename.endsWith(".mp4")
      ? filename
...
export async function downloadVideoAsMP3(
  ...
  try {
    ...
    if (
      ...
      !videoAny.webkitAudioDecodedByteCount &&
