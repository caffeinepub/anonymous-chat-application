...
import { Shield, Users, Loader2, User } from 'lucide-react';
import { useCreateRoom, useCheckRoomExists, useJoinRoom } from '../hooks/useQueries';
...
export default function WelcomeScreen({ onJoinRoom }: WelcomeScreenProps) {
  ...
  const { refetch: checkRoomExists } = useCheckRoomExists(normalizeRoomId(joinCode));
  ...
  const handleCreateRoom = async (e: React.FormEvent) => {
    ...
    try {
      console.log('[WelcomeScreen] Creating room with code:', normalized, 'nickname:', trimmedNickname);
      const roomId = await createRoomMutation.mutateAsync({ joinCode: normalized, nickname: trimmedNickname });
  ...
  const handleJoinRoom = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (exists) {
        ...
        await joinRoomMutation.mutateAsync({ joinCode: normalized, nickname: trimmedNickname });
      ...
      <div className="w-full max-w-4xl space-y-8">
        ...
        <div className="text-center space-y-4">
          ...
          <p className="text-muted-foreground text-lg max-w-md mx-auto">
            Private, secure messaging rooms. Enter a nickname to create or join a room.
          </p>
        </div>
        ...
        <div className="grid md:grid-cols-2 gap-6">
          ...
          <Card>
            <CardHeader>
              ...
              <CardDescription>Start a new secure chat room with a custom code</CardDescription>
            </CardHeader>
            <CardContent>
              <form onSubmit={handleCreateRoom} className="space-y-4">
                <div className="space-y-2">
                  ...
                  <p className="text-xs text-muted-foreground">Choose a display name (max 20 characters)</p>
                </div>
                <div className="space-y-2">
                  ...
                  <Input
                    ...
                    disabled={createRoomMutation.isPending}
                  />
                  <p className="text-xs text-muted-foreground">Choose a memorable code (max 30 characters)</p>
                </div>
                <Button
                  type="submit"
                  className="w-full"
                  disabled={!createCode.trim() || !createNickname.trim() || createRoomMutation.isPending}
                >
              ...
              <form onSubmit={handleJoinRoom} className="space-y-4">
                ...
                <div className="space-y-2">
                  ...
                  <Input
                    ...
                    disabled={joinRoomMutation.isPending}
                  />
                  <p className="text-xs text-muted-foreground">Ask the room creator for the code</p>
                </div>
                <Button
                  type="submit"
                  className="w-full"
                  disabled={!joinCode.trim() || !joinNickname.trim() || joinRoomMutation.isPending}
                >
        ...
        <div className="grid sm:grid-cols-3 gap-4 pt-8">
          ...
          <div className="text-center space-y-2">
            ...
            <p className="text-sm text-muted-foreground">Create custom rooms with unique codes</p>
          </div>
          <div className="text-center space-y-2">
            <div className="w-12 h-12 rounded-full bg-primary/10 flex items-center justify-center mx-auto">
            ...
            <p className="text-sm text-muted-foreground">No account required, just a nickname</p>
          </div>
        </div>
      </div>
    </div>
